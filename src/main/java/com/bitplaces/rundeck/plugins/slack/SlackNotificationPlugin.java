...
@PluginDescription(title = "Slack Notification Incoming WebHook", description = "Sends Rundeck Notifications to Slack")
public class SlackNotificationPlugin implements NotificationPlugin {
    ...
    @PluginProperty(title = "WebHook Base URL", description = "Slack Incoming WebHook Base URL", defaultValue = "https://hooks.slack.com/services", scope = PropertyScope.Instance)
    private String webhook_base_url;

    @Password
    @PluginProperty(title = "WebHook Token", description = "WebHook Token, like T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX", scope = PropertyScope.Instance)
    private String webhook_token;

    @PluginProperty(title = "Slack Channel", description = "Slack Channel, like #channel-name (optional)", scope = PropertyScope.Instance)
    private String slack_channel;

    /**
     * Sends a message to a Slack room when a job notification event is raised by
     * Rundeck.
     *
     * @param trigger       name of job notification event causing notification
     * @param executionData job execution data
     * @param config        plugin configuration
     * @throws SlackNotificationPluginException when any error occurs sending the
     *                                          Slack message
     * @return true, if the Slack API response indicates a message was successfully
     *         delivered to a chat room
     */
    public boolean postNotification(String trigger, Map executionData, Map config) {
        ...
        if ("ok".equals(slackResponse)) {
            ...
            // logger within notification plugins,
    ...
    public String invokeSlackAPIMethod(String webhook_url, String message) {
